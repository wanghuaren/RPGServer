...
{
    /// <summary>
    /// 大鹏竞速, 每天晚上20点刷新, 21点结束
    /// </summary>
    [CollectionAgeLimit(AlwaysActive = true)]
    public class EagleRaceGrain : Grain, IEagleRaceGrain
    {
        ...
        private ILogger<EagleRaceGrain> _logger;

        private const string Name = "大鹏竞速";

        public EagleRaceGrain(ILogger<EagleRaceGrain> logger)
        {
            _logger = logger;
        }
        ...
        {
            _logger?.LogInformation($"大鹏竞速[{_serverId}]:{msg}");
        }

        private void LogDebug(string msg)
        {
            _logger?.LogDebug($"大鹏竞速[{_serverId}]:{msg}");
        }

        private void LogError(string msg)
        {
            _logger?.LogError($"大鹏竞速[{_serverId}]:{msg}");
        }
